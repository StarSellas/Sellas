	...
	<section class="py-5">
		...
		<div class="container px-4 px-lg-5 mt-5 tradecontainter"
			...
			<div
				...
				<div id="detail">
					<input type="hidden" value="${detail.mnickname}"
						class="sellerMnickname"> <input type="hidden"
						value="${sessionScope.mnickname }" class="mnickname"> <input
						type="hidden" value="${detail.tno }" class="normalTno"> <input
						type="hidden" value="${detail.muuid }" class="normalMuuid">
					<input type="hidden" value="${detail.tnormalprice }"
						class="tnormalprice"> 세션값이오나용 ${sessionScope.mnickname }
					여기에 수정삭제를 만들까 아래에 수정삭제를 만들까 고민해봅시다

					<h2>제목 : ${detail.ttitle} , 상태 : ${detail.tnormalstate}</h2>
					<br> 가격 : ${detail.tnormalprice } 웨일 페이
					...
					<form action="/chat/onlyalarm" method="post">
						...
						<button type="submit" id="requestChatBtn">채팅 신청</button>
						되면 위 if문으로
					</form>
					<c:if
						test="${sessionScope.muuid != detail.muuid &&detail.tnormalstate == 1}">
							거래중입니다.
					</c:if>
					<c:if
						test="${sessionScope.muuid != detail.muuid &&detail.tnormalstate == 2}">
							거래가 완료된 물품입니다.
					</c:if>
				</div>
			</div>
			...
			if (confirm("거래 신청 하시겠습니까?")) {
				...
				$.ajax({
					...
					data : {
						tno : tno,
						mnickname : mnickname,
						tnormalprice : tnormalprice,
						sellerMnickname : sellerMnickname
					},
					dataType : "json",
					success : function(data) {
						alert("ㅎㅎ");
